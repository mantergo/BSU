...
//!!!!!!!!!!!!!!!!!!! KONEC BIBLIOTEKI INFINT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!


using namespace std;

const unsigned long long N = 21;
char tab[N][N], tempTab[N][N];
InfInt dp[N][1 << 11];
unsigned long long n, m;

bool bit(unsigned long long pos, unsigned long long mask) {
    return mask & (1 << pos);
}

void calc(unsigned long long i, unsigned long long j, unsigned long long pos, unsigned long long mask) {
    if ((bit(pos, j) && tab[pos][i] == '1')) {
        return;
    }
    if (pos == n) {
        dp[i+1][mask] += dp[i][j];
    ...
    if (bit(pos, j) || tab[pos][i] == '1') {
        ...
        if (i + 1 < m && tab[pos][i + 1] != '1') {
            calc(i, j, pos + 1, mask | (1 << pos));
        }
        if (pos < n - 1 && !bit(pos + 1, j) && (tab[pos + 1][i] != '1')) {
            calc(i, j, pos + 2, mask);
        }
    }
}
